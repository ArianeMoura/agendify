...
export default function EditSpaceScreen() {
  ...
  const { data: space, isLoading: spaceLoading } = useQuery({
    queryKey: ['spaces', id],
    queryFn: () => spacesApi.getById(id as string),
    enabled: !!id,
  });

  useEffect(() => {
    if (space?.imageUrl) {
      setImageUri(getImageUrl(space.imageUrl) || '');
    }
    if (space?.availableHours) {
      setSelectedHours(space.availableHours);
    }
  }, [space]);
  ...
  const {
    ...
    defaultValues: {
      ...
      allDayStartTime: space?.allDayStartTime || '11:00',
      allDayEndTime: space?.allDayEndTime || '22:00',
