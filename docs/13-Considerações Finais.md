...
A arquitetura distribuída atendeu plenamente aos objetivos funcionais do projeto. Contudo, uma análise crítica revela pontos de evolução para um cenário de mercado real, com destaque para dois desafios centrais: garantir a integridade dos dados em reservas simultâneas (controle de concorrência) e manter a escalabilidade da arquitetura distribuída para suportar picos de acesso.
...
- **Análise do Processo:** O ritmo das Sprints permitiu entregas incrementais (MVP), o que foi essencial para identificar erros de integração entre o Mobile e a API cedo no processo. As dailies, mesmo que assíncronas em alguns momentos, mantiveram a equipe alinhada.
...
- Configuração de containerização com Docker (`docker-compose.yml`), padronizando a execução local da API e do banco de dados;
- Rotina de integração contínua no GitHub Actions (verificação de segurança do código);
